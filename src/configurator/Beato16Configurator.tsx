...
interface Beato16ConfiguratorProps {
  currentUser: User;
  onLogout: () => void;
}

const Beato16Configurator: React.FC<Beato16ConfiguratorProps> = ({ currentUser, onLogout }) => {
  ...
  const handleFinalizeOpenModal = useCallback(async () => {
    ...
    if (result.isConfirmed) {
      try {
        // 1) Usar captura guardada en preConfirm; fallback
        ...
        const subject = 'Beato16 Configuration';
        const normalizeName = (name: string) => name.replace(/Boton/gi, 'Button').replace(/tecla/gi, 'Key').replace(/fader/gi, 'Fader').replace(/_\d+\b/i, '');
        const spanishToEnglish: Record<string, string> = { 'Verde': 'Green','Amarillo': 'Yellow','Azul': 'Blue','Blanco': 'White','Naranja': 'Orange','Morado': 'Purple','Rojo': 'Red','Negro': 'Black','Rosa': 'Pink','Gris': 'Gray' };
        const toEnglish = (n: string) => spanishToEnglish[n] || n;
  ...
  const prepareModelParts = useCallback((model: THREE.Group) => {
    ...
    // Siempre usar colores por defecto al cargar el modelo
